  ...
  if (snapshots.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-12 text-slate-400">
        ...
        <h3 className="text-lg font-bold mb-2">{t.journalNoHistory || '暂无决策记录'}</h3>
        <p className="text-sm text-center max-w-md">
          {t.journalCTA || '在上方点击 "保存当前决策" (Save Decision) 按钮，记录下您此刻的思考、放弃的选项以及市场判断。\n这将帮助您在未来复盘时，避免“事后诸葛亮”，真正提高决策质量。'}
        </p>
      </div>
    );
  }
    ...
    <div className="flex flex-col md:flex-row gap-6 h-[600px] animate-fade-in">
      ...
      <div className="w-full md:w-1/3 bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-4 overflow-y-auto border border-slate-100 dark:border-slate-800">
        <h3 className="text-sm font-bold text-slate-500 mb-4 flex items-center gap-2">
          <History className="h-4 w-4" /> {t.journalTimeline || '决策时间轴'}
        </h3>
          ...
          {snapshots.map(s => (
            ...
            >
              <div className="flex justify-between items-start mb-2">
                ...
                {s.tags?.includes('High Risk') && <span className="text-[10px] bg-rose-100 text-rose-600 px-1.5 py-0.5 rounded-full">High Risk</span>}
              </div>
              <div className="font-bold text-slate-800 dark:text-white text-sm mb-1 line-clamp-1">
                {s.userNotes || t.journalNoNotes || '...'}
              </div>
              <div className="text-xs text-slate-500 flex gap-2">
                <span>{t.totalPrice || '总价'}: {s.params.totalPrice}{t.unitWanSimple || '万'}</span>
                <span>{t.interestRate || '利率'}: {s.params.interestRate}%</span>
              </div>
            </div>
          ))}
        </div>
          ...
          <>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div className="grid grid-cols-4 gap-4 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-800 text-center">
                 <div>
                    <div className="text-xs text-slate-500 mb-1">{t.totalPrice || '房产总价'}</div>
                    <div className="font-mono font-bold">{selectedSnapshot.params.totalPrice}{t.unitWanSimple || '万'}</div>
                 </div>
                 <div>
                    <div className="text-xs text-slate-500 mb-1">{t.downPaymentRatio || '首付比例'}</div>
                    <div className="font-mono font-bold">{selectedSnapshot.params.downPaymentRatio}%</div>
                 </div>
                 <div>
                    <div className="text-xs text-slate-500 mb-1">{t.interestRate || '贷款利率'}</div>
                    <div className="font-mono font-bold">{selectedSnapshot.params.interestRate}%</div>
                 </div>
                 <div>
                    <div className="text-xs text-slate-500 mb-1">{t.marketSentiment || '市场情绪'}</div>
              ...
              <div className="space-y-4">
                 <div className="flex justify-between items-center">
                    <h3 className="font-bold text-slate-700 dark:text-slate-200 flex items-center gap-2">
                      <CheckCircle2 className="h-4 w-4 text-emerald-500" /> {t.journalReason || '为什么做这个决定？'}
                    </h3>
                    {!editMode ? (
                      <button onClick={() => setEditMode(true)} className="text-xs text-indigo-500 hover:underline flex items-center gap-1">
                        <Edit3 className="h-3 w-3" /> {t.journalEdit || '编辑'}
                      </button>
                    ) : (
                 ...
                 {editMode ? (
                   <textarea
                     ...
                     placeholder={t.journalPlaceholder || '记录你的核心理由，比如：学区房保值、急着结婚、担心踏空...'}
                   />
                 ) : (
                   <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-800 text-sm text-slate-700 dark:text-slate-300 leading-relaxed whitespace-pre-wrap">
                     {selectedSnapshot.userNotes || <span className="text-slate-400 italic">...</span>}
                   </div>
                 )}
              </div>
              ...
              <div className="space-y-4">
                 <h3 className="font-bold text-slate-700 dark:text-slate-200 flex items-center gap-2">
                    <XCircle className="h-4 w-4 text-rose-500" /> {t.journalRejected || '我放弃了什么？（机会成本）'}
                 </h3>
                 {editMode ? (
                   <textarea
                     className="w-full h-24 p-3 bg-slate-50 dark:bg-slate-800 border border-indigo-300 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 outline-none resize-none"
                     ...
                     placeholder={t.journalRejectedPlaceholder || '比如：更远但更大的新房、继续租房买理财、老破小...'}
                   />
                 ) : (
                   <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-800 text-sm text-slate-700 dark:text-slate-300 leading-relaxed whitespace-pre-wrap">
                     {selectedSnapshot.rejectedOptions || <span className="text-slate-400 italic">...</span>}
                   </div>
                 )}
              </div>
              ...
              <div className="border-t border-slate-100 dark:border-slate-800 pt-6 mt-6">
                 <div className="flex justify-between items-center mb-4">
                    <h3 className="font-bold text-slate-700 dark:text-slate-200 flex items-center gap-2">
                       <BrainCircuit className="h-5 w-5 text-indigo-600" /> {t.journalAIReview || 'AI 决策复盘'}
                    </h3>
                    <button 
                    ...
                    >
                      {isAnalyzing ? (t.journalAnalyzing || '分析中...') : (t.journalAnalyzeBtn || '开始复盘分析')}
                    </button>
                 </div>
                 ...
                 {selectedSnapshot.aiFeedback ? (
                   <div className="bg-indigo-50 dark:bg-indigo-900/10 p-5 rounded-2xl border border-indigo-100 dark:border-indigo-800/30">
                      <div className="prose prose-sm dark:prose-invert max-w-none">
                         <div dangerouslySetInnerHTML={{ __html: (marked.parse(selectedSnapshot.aiFeedback) as string) }} />
                      </div>
                   </div>
                 ) : (
                   <div className="text-center py-8 text-slate-400 bg-slate-50 dark:bg-slate-800/30 rounded-2xl border border-dashed border-slate-200 dark:border-slate-700">
                      {t.journalAICallout || '点击右上角按钮，让 AI 帮你检查决策盲点。'}
                      <br/>
                      <span className="text-xs opacity-70">AI Check</span>
                   </div>
                 )}
              </div>
