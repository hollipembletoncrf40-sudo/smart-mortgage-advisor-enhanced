...
const RiskHeartbeatChart: React.FC<RiskHeartbeatChartProps> = ({ result, params, t }) => {
  ...
  return (
    <div className="bg-slate-900 rounded-3xl p-6 shadow-xl border border-slate-800 relative overflow-hidden text-white transition-colors duration-500">
      <div className="flex items-center justify-between mb-4 relative z-10">
        <h3 className="text-sm font-bold flex items-center gap-2 text-emerald-400">
          <Activity className="h-4 w-4" />
          {t.financialEKG || '财务波动心电图'}
        </h3>
        <div className="flex items-center gap-2 bg-slate-800/50 rounded-lg p-1 px-2">
            <span className="text-xs text-slate-400">{t.riskAversion || '风险厌恶度'}:</span>
            <div className="flex gap-1">
                {[1, 2, 3, 4, 5].map(star => (
                    <button 
                        key={star}
                        onClick={() => setRiskAversion(star)}
                        className={`text-sm transition-transform hover:scale-110 ${star <= riskAversion ? 'text-red-500' : 'text-slate-600'}`}
      ...
      {/* Chart */}
      <div className={`h-40 w-full rounded-xl border relative transition-colors duration-500 ${isTachycardia ? 'bg-red-950/10 border-red-900/30' : 'bg-emerald-950/10 border-emerald-900/30'}`}>
        ...
        <div className="absolute top-2 right-2 flex flex-col items-end pointer-events-none">
             <div className="text-xs font-mono text-slate-400 mb-1">
                {t.currentVolatility || '当前波动'}: <span className={volatilityScore > 50 ? 'text-orange-400' : 'text-emerald-400'}>{Math.round(volatilityScore)}</span>
             </div>
      ...
      <div className="mt-4">
        {isTachycardia ? (
            <div className="flex items-start gap-3 bg-red-900/20 p-3 rounded-xl border border-red-900/50 animate-in slide-in-from-bottom-2">
                <div className="p-2 bg-red-900/50 rounded-full animate-pulse">
                    <ShieldAlert className="h-5 w-5 text-red-400" />
                </div>
                <div>
                    <div className="text-sm font-bold text-red-400 mb-1">{t.tachycardiaWarning || '心率过速警告！'}</div>
            ...
            <div className="flex items-start gap-3 bg-emerald-900/20 p-3 rounded-xl border border-emerald-900/50 animate-in slide-in-from-bottom-2">
                <div className="p-2 bg-emerald-900/50 rounded-full">
                    <ShieldCheck className="h-5 w-5 text-emerald-400" />
                </div>
                <div>
                    <div className="text-sm font-bold text-emerald-400 mb-1">{t.healthNormal || '心率正常'}</div>
