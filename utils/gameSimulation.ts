...
const EVENTS: Omit<GameEvent, 'year'>[] = [
...
export const simulateLife = (params: InvestmentParams): GameResult => {
  ...
  let summary = '你度过了平凡而充实的一生。';
  
  if (wealth < 0 || stress > 90) {
    ending = 'DEBT';
    summary = '高额的房贷和生活压力让你喘不过气，最终陷入了财务危机。';
  } else if (wealth > params.totalPrice * 3) {
    ending = 'WEALTHY';
    summary = '你的资产在20年间翻了几番，实现了财务自由！';
  } else if (happiness > 85 && stress < 40) {
    ending = 'ZEN';
    summary = '虽然不是大富大贵，但你心态平和，生活幸福指数极高。';
  } else if (stress > 70 && wealth > params.totalPrice) {
    ending = 'STRUGGLE';
    summary = '虽然积累了不错的资产，但为了还房贷，你牺牲了太多的生活质量。';
  }
